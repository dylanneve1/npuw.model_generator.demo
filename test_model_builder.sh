...
for config_entry in "${CONFIGS[@]}"; do
    ...
    for backend_entry in "${ACTIVE_BACKENDS[@]}"; do
        ...
        if [[ $IS_WHISPER -eq 1 ]]; then
            ...
            # Decoder embedding (Qwen3-style): llm_bench text_embed on both CPU and NPU
            if [[ "$BACKEND_NAME" == "CPU" ]]; then
            ...
            # Encoder embedding (BERT-style): llm_bench on CPU, bare-OV on NPU (needs static shapes)
            if [[ "$BACKEND_NAME" == "CPU" ]]; then
                BENCH_CMD="bash -c 'source $VENV && source $SETUPVARS && python $LLM_BENCH -m $MODEL_DIR --task text_embed -d CPU -n 3'"
            else
