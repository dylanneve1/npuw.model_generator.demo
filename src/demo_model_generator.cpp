...
// Outputs OpenVINO IR models compatible with llm_bench/LLMPipeline and WhisperPipeline.
//
...
static void print_help(const char* prog) {
    std::cout << "Usage: " << prog << " --type <llm|whisper|embedding> [options]\n\n";
    std::cout << "Generate synthetic OpenVINO IR models for NPUW testing.\n\n";
    std::cout << "Required:\n";
    std::cout << "  --type <type>           Model type: llm, whisper, embedding\n\n";
    std::cout << "Output:\n";
    ...
    std::cout << "Tokenizer:\n";
    std::cout << "  -t, --tokenizer <dir>   Copy tokenizer files from an existing model directory\n\n";
    std::cout << "LLM options:\n";
    std::cout << "  --num-layers <N>        Number of decoder layers (default: 12)\n";
    ...
    std::cout << "  --context-len <N>       Max context length for RoPE (default: 128)\n";
    ...
    std::cout << "  --weight-type <type>    Weight format: fp32, fp16, int8, int4 (default: fp32)\n";
    std::cout << "  --group-size <N>        Group size for int4 group quantization, 0=per-channel (default: 0)\n";
    std::cout << "  --inputs-embeds         Use inputs_embeds instead of input_ids (VLM-style)\n";
    std::cout << "  --position-ids <type>   Position IDs shape: 2d, 3d, none (default: 2d)\n";
    ...
    std::cout << "  --ffn-dim <N>           FFN intermediate dimension (default: 1536)\n";
    std::cout << "  --vocab-size <N>        Vocabulary size (default: 51865)\n";
    std::cout << "  --weight-type <type>    Weight format: fp32, fp16 (default: fp32)\n\n";
    std::cout << "Embedding options:\n";
    std::cout << "  --arch <type>           Architecture: decoder (Qwen3-style), encoder (BERT-style)\n";
    std::cout << "                          (default: decoder)\n";
    std::cout << "  (decoder arch also accepts all LLM options above)\n";
    std::cout << "  (encoder arch also accepts --hidden-size, --num-heads, --head-dim,\n";
    std::cout << "   --intermediate-size, --vocab-size, --num-layers, --weight-type)\n\n";
    std::cout << "General:\n";
    std::cout << "  -h, --help              Show this help\n";
}

static void write_config_json(const fs::path& dir, const LLMConfig& config,
                              size_t context_len, const std::string& model_type_id) {
    bool qwen_tokens = (model_type_id == "qwen2_5_vl" || model_type_id == "llava");
    {
...
static void write_embedding_config_json(const fs::path& dir, const LLMConfig& config, size_t context_len) {
    std::ofstream ofs(dir / "config.json");
    ofs << "{\n";
...
static void write_encoder_config_json(const fs::path& dir, const BERTConfig& config) {
    std::ofstream ofs(dir / "config.json");
    ofs << "{\n";
    ofs << "  \"model_type\": \"bert\",\n";
    ofs << "  \"hidden_size\": " << config.hidden_size << ",\n";
    ...
    ofs << "  \"intermediate_size\": " << config.intermediate_size << ",\n";
    ofs << "  \"vocab_size\": " << config.vocab_size << ",\n";
    ofs << "  \"max_position_embeddings\": 512,\n";
    ofs << "  \"pad_token_id\": 0\n";
    ofs << "}\n";
}

static void copy_tokenizer_files(const fs::path& src_dir, const fs::path& dst_dir) {
...
static void symlink_vlm_assets(const fs::path& src_dir, const fs::path& dst_dir) {
...
static void write_whisper_configs(const fs::path& dir, const WhisperConfig& config) {
    {
        std::ofstream ofs(dir / "config.json");
        ofs << "{\n";
        ...
        ofs << "  \"d_model\": " << config.d_model << ",\n";
        ofs << "  \"encoder_layers\": " << config.encoder_layers << ",\n";
        ofs << "  \"encoder_attention_heads\": " << config.encoder_attention_heads << ",\n";
        ofs << "  \"encoder_ffn_dim\": " << config.encoder_ffn_dim << ",\n";
        ofs << "  \"decoder_layers\": " << config.decoder_layers << ",\n";
        ofs << "  \"decoder_attention_heads\": " << config.decoder_attention_heads << ",\n";
        ofs << "  \"decoder_ffn_dim\": " << config.decoder_ffn_dim << ",\n";
        ofs << "  \"vocab_size\": " << config.vocab_size << ",\n";
...
int main(int argc, char* argv[]) {
    ...
    // Whisper config (defaults match whisper-tiny)
    WhisperConfig whisper_config;

    // CLI-only settings (not in LLMConfig)
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help" || arg == "-h") {
            ...
            if (ffn_type_str != "swiglu" && ffn_type_str != "gelu") {
                std::cerr << "Error: unknown --ffn-type '" << ffn_type_str << "' (expected: swiglu, gelu)\n";
                return 1;
            }
        } else if (arg == "--norm-type" && i + 1 < argc) {
            norm_type_str = argv[++i];
            if (norm_type_str != "rms" && norm_type_str != "layer") {
                std::cerr << "Error: unknown --norm-type '" << norm_type_str << "' (expected: rms, layer)\n";
                return 1;
            }
        } else if (arg == "--rope-type" && i + 1 < argc) {
            rope_type_str = argv[++i];
            if (rope_type_str != "half" && rope_type_str != "interleaved" && rope_type_str != "none") {
            ...
            if (position_ids_str != "2d" && position_ids_str != "3d" && position_ids_str != "none") {
            ...
            if (arch_str != "decoder" && arch_str != "encoder") {
                std::cerr << "Error: unknown --arch '" << arch_str << "' (expected: decoder, encoder)\n";
                return 1;
            }
    ...
    if (model_type != "llm" && model_type != "whisper" && model_type != "embedding") {
    ...
    if (model_type == "whisper") {
        // --num-heads and --vocab-size are shared with LLM parsing
        if (config.num_heads != 8) {
            whisper_config.encoder_attention_heads = config.num_heads;
            whisper_config.decoder_attention_heads = config.num_heads;
        }
        if (config.vocab_size != 32000)
            whisper_config.vocab_size = config.vocab_size;

        WeightFn wf;
        if (weight_type_str == "fp16")  wf = FP16Weight{};
        else                            wf = FP32Weight{};
        whisper_config.weight = wf;

        std::cout << "Generating Whisper model:\n";
        std::cout << "  d_model:           " << whisper_config.d_model << "\n";
        std::cout << "  encoder_layers:    " << whisper_config.encoder_layers << "\n";
        std::cout << "  decoder_layers:    " << whisper_config.decoder_layers << "\n";
        std::cout << "  attention_heads:   " << whisper_config.encoder_attention_heads << "\n";
        std::cout << "  ffn_dim:           " << whisper_config.encoder_ffn_dim << "\n";
        std::cout << "  vocab_size:        " << whisper_config.vocab_size << "\n";
        ...
        auto encoder = mb.build_whisper_encoder(whisper_config);
        ov::pass::Serialize enc_serializer((dest / "openvino_encoder_model.xml").string(),
                                           (dest / "openvino_encoder_model.bin").string());
        enc_serializer.run_on_model(encoder);
        std::cout << "Encoder model saved.\n";

        auto decoder = mb.build_whisper_decoder(whisper_config);
        ov::pass::Serialize dec_serializer((dest / "openvino_decoder_model.xml").string(),
                                           (dest / "openvino_decoder_model.bin").string());
        dec_serializer.run_on_model(decoder);
        std::cout << "Decoder model saved.\n";

        write_whisper_configs(dest, whisper_config);

        if (!tokenizer_dir.empty()) {
            copy_tokenizer_files(tokenizer_dir, dest);
            auto norm_src = fs::path(tokenizer_dir) / "normalizer.json";
            if (fs::exists(norm_src)) {
                fs::copy_file(norm_src, dest / "normalizer.json", fs::copy_options::overwrite_existing);
            }
        }
    ...
    if (model_type == "embedding") {
        if (arch_str == "encoder") {
            // BERT-style encoder (e.g. Contriever)
            BERTConfig bert_config;
            bert_config.hidden_size = config.hidden_size;
            bert_config.num_heads = config.num_heads;
            bert_config.head_dim = config.head_dim;
            bert_config.intermediate_size = config.intermediate_size;
            bert_config.vocab_size = config.vocab_size;
            bert_config.num_layers = config.num_layers;

            WeightFn wf;
            if (weight_type_str == "fp32")       wf = FP32Weight{};
            else if (weight_type_str == "fp16")  wf = FP16Weight{};
            ...
            std::cout << "  hidden_size:       " << bert_config.hidden_size << "\n";
            std::cout << "  num_heads:         " << bert_config.num_heads << "\n";
            std::cout << "  head_dim:          " << bert_config.head_dim << "\n";
            std::cout << "  intermediate_size: " << bert_config.intermediate_size << "\n";
            std::cout << "  vocab_size:        " << bert_config.vocab_size << "\n";
            std::cout << "  weight:            " << weight_type_str << "\n";
            if (!tokenizer_dir.empty())
                std::cout << "  tokenizer_src:     " << tokenizer_dir << "\n";
            std::cout << "\n";
            ...
            ModelBuilder mb;
            auto model = mb.build_bert_encoder(bert_config);

            ov::pass::Serialize serializer((dest / "openvino_model.xml").string(),
                                           (dest / "openvino_model.bin").string());
            serializer.run_on_model(model);

            write_encoder_config_json(dest, bert_config);
        } else {
            // Decoder-only embedding (e.g. Qwen3-Embedding): LLM without lm_head, no KV cache
            config.use_kv_cache = false;
            ...
            WeightFn weight_fn;
            if (weight_type_str == "fp32")       weight_fn = FP32Weight{};
            else if (weight_type_str == "fp16")  weight_fn = FP16Weight{};
            else if (weight_type_str == "int8")  weight_fn = INT8Weight{};
            else if (weight_type_str == "int4") {
                if (group_size > 0) weight_fn = INT4GroupWeight{group_size};
                else                weight_fn = INT4Weight{};
            }
            ...
            // rope left empty — build_llm auto-creates HalfRotationRoPE with internal position_ids

            if (ffn_type_str == "swiglu")
                config.ffn = SwiGLU(config.hidden_size, config.intermediate_size, config.precision, weight_fn);
            else
                config.ffn = GELUFn(config.hidden_size, config.intermediate_size, config.precision, weight_fn);
    ...
    // Validate group_size usage
    if (group_size > 0 && weight_type_str != "int4") {
        std::cerr << "Error: --group-size is only supported with --weight-type int4\n";
        return 1;
    }

    // --- Configure LLM functors from CLI strings ---
    WeightFn weight_fn;
    if (weight_type_str == "fp32")       weight_fn = FP32Weight{};
    else if (weight_type_str == "fp16")  weight_fn = FP16Weight{};
    else if (weight_type_str == "int8")  weight_fn = INT8Weight{};
    ...
    // Position IDs & RoPE: position_ids must be created before RoPE (baked into constructor)
    if (rope_type_str != "none" && position_ids_str != "none") {
        if (position_ids_str == "3d")
            config.position_ids = make_position_ids_3d();
        // "2d" = default; build_llm auto-creates 2D Parameter

        if (rope_type_str == "interleaved") {
            // Interleaved requires explicit construction with position_ids
            if (!config.position_ids.get_node())
                config.position_ids = make_position_ids_2d();
            config.rope = InterleavedRoPE(config.head_dim, config.precision, config.position_ids);
        }
        // "half" = default; build_llm auto-creates HalfRotationRoPE
    } else {
        // No RoPE: set identity pass-through to prevent build_llm from auto-creating RoPE
        config.rope = [](const ov::Output<ov::Node>& input, const std::string&) { return input; };
    }
    ...
    std::cout << "Generating " << (config.use_inputs_embeds ? "VLM" : "LLM") << " model:\n";
    std::cout << "  layers:            " << config.num_layers << "\n";
    std::cout << "  hidden_size:       " << config.hidden_size << "\n";
    std::cout << "  num_heads:         " << config.num_heads << "\n";
    std::cout << "  num_kv_heads:      " << config.get_kv_heads()
    ...
    std::cout << "  inputs_embeds:     " << (config.use_inputs_embeds ? "yes" : "no") << "\n";
    ...
    if (config.use_inputs_embeds) {
        // VLM mode: save as language model + separate text embeddings model
        ov::pass::Serialize lang_serializer((dest / "openvino_language_model.xml").string(),
                                            (dest / "openvino_language_model.bin").string());
        lang_serializer.run_on_model(model);

        // Build text embeddings model: input_ids -> Gather(embedding) -> inputs_embeds
        ...
        auto emb_output = make_embedding(emb_input->output(0), config.vocab_size,
                                          config.hidden_size, "model.embed_tokens", config.precision);
        emb_output.set_names({"inputs_embeds"});

        auto emb_result = std::make_shared<ov::op::v0::Result>(emb_output);

        auto emb_model = std::make_shared<ov::Model>(
            ov::ResultVector{emb_result},
            ov::ParameterVector{emb_input},
            "text_embeddings_model");

        ov::pass::Serialize emb_serializer((dest / "openvino_text_embeddings_model.xml").string(),
                                            (dest / "openvino_text_embeddings_model.bin").string());
        emb_serializer.run_on_model(emb_model);

        // Symlink vision models from source model directory
        if (!tokenizer_dir.empty()) {
            symlink_vlm_assets(tokenizer_dir, dest);
        }
    } else {
        // Standard LLM: save as openvino_model
        ov::pass::Serialize serializer((dest / "openvino_model.xml").string(),
                                       (dest / "openvino_model.bin").string());
        serializer.run_on_model(model);
    }

    // Determine model_type for config.json
    ...
    // Copy tokenizer from existing model if specified
    if (!tokenizer_dir.empty()) {
        copy_tokenizer_files(tokenizer_dir, dest);
    }
